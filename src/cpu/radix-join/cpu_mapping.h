...
static int memmapping[MAX_THREADS];
 ...
 */
static int
init_mappings_from_file(const char * filename, int *mapping)
{
    FILE * cfg;
  int i;
...
numa_init()
{
  ...
  struct bitmask *cpus;
    ...
    fprintf(stderr, "[INFO ] Getting the NUMA configuration automatically with libNUMA. \n");
...
#else
    fprintf(stderr, "[WARN ] NUMA is not available, using single NUMA-region as default.\n");
    numa_default_init();
#endif
}
...
cpu_mapping_init()
{
    max_cpus  = sysconf(_SC_NPROCESSORS_ONLN);
     if( init_mappings_from_file(CUSTOM_CPU_MAPPING, cpumapping) == 0 ) {
         int i;
...
{
    max_cpus  = sysconf(_SC_NPROCESSORS_ONLN);
     if( init_mappings_from_file(CUSTOM_MEM_MAPPING, memmapping) == 0 ) {
         int i;
...
int
get_mem_id(int thread_id)
{
  if (!mem_inited)
    mem_mapping_init();
  return memmapping[thread_id % max_threads];
}
...
get_cpu_id(int thread_id)
{
    ...
    cout << cpumapping[thread_id % max_threads] << endl;
