...
#define USE_SWWC_OPTIMIZED_PART
...
// static
// join_result_t
// robin_hood_join(const relation_t * const R,
//         const relation_t * const S,
//         relation_t * const tmpR, uint64_t totalNumR)
// {
//   const uint64_t numR = R->num_tuples;
...
//     uint64_t N_old=N;
//     NEXT_POW_2(N);
//     if ((double)N_old/N>MAXLOADFACTOR) {
//         N=N<<1;
//     }


//   LinearHashTable<intkey_t, value_t, IdHash<NUM_RADIX_BITS>, 0L,false> ht(cnt_power(N));
//   for (uint64_t i = 0; i < numR; ++i)
//     ht.put(tupleR[i].key, tupleR[i].payload);

//   typename LinearHashTable<intkey_t, value_t, IdHash<NUM_RADIX_BITS>, 0L, false>::Result found_tuple;
//   for (uint64_t i = 0; i < numS; ++i)
//   {
//     found_tuple = ht.get(tupleS[i].key);
//     if (found_tuple.found)
//     {
//       ++matches;
//       if (is_checksum)
//       {
//         checksum += found_tuple.value+ tupleS[i].payload;
//       }
//     }
//   }
...
prj_thread(void * param)
{
  ...
  double partition_time = diff.count() * 1000;

  BARRIER_ARRIVE(args->barrier, rv);

  if (my_tid == 0) {
    cout << "PT: " << partition_time << endl;
  }
...
// #endif
    ...
    cout << "Created " << nthreads << " Threads" << endl;
...
int main(int argc, char** argv) {
  ...
  //for (int i=0; i<5; i++) cout << relS.tuples[i].key << " " << relS.tuples[i].payload << endl;

  join_result_t res = PRAiS<true, 7, 1>(&relR, &relS, nthreads);
