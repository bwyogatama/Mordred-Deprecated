...
/*
Implementation of q32
select c_city,s_city,d_year,sum(lo_revenue) as revenue from lineorder,customer,supplier,date where lo_custkey = c_custkey and lo_suppkey = s_suppkey and lo_orderdate = d_datekey and c_nation = 'UNITED STATES' and s_nation = 'UNITED STATES' and d_year >=1992 and d_year <= 1997 group by c_city,s_city,d_year order by d_year asc,revenue desc;
*/
...
float runQuery(int* lo_orderdate, int* lo_custkey, int* lo_suppkey, int* lo_revenue, int lo_len,
        ...
        if (slot != 0) {
          ...
          if (slot != 0) {
            ...
            if (slot != 0) {
              ...
              hash = (s_nation * 250 * 7 + c_nation * 7 + (year - 1992)) % num_slots;
              res[hash].year = year;
  ...
  for (int i=0; i<num_slots; i++) {
    if (res[i].year != 0) {
      cout << res[i].year << " " << res[i].c_nation << " " << res[i].s_nation << " " << res[i].revenue << endl;
      res_count += 1;
    }
  }
...
int main(int argc, char** argv) {
  ...
  for (int t = 0; t < num_trials; t++) {
    ...
    time_query = runQuery(
        ...
        c_custkey, c_nation, c_city, C_LEN);
    cout<< "{"
        << "\"query\":32"
