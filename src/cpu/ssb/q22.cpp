...
/*
Implementation of q22
select sum(lo_revenue),d_year,p_brand1 from lineorder, part, supplier,date where lo_orderdate = d_datekey and lo_partkey = p_partkey and lo_suppkey = s_suppkey and p_brand1 >= 'MFGR#2221' and p_brand1 <= 'MFGR#2228' and s_region = 'ASIA' group by d_year,p_brand1 order by d_year,p_brand1;
*/
...
float runQuery(int* lo_orderdate, int* lo_partkey, int* lo_suppkey, int* lo_revenue, int lo_len,
  ...
  parallel_for(blocked_range<size_t>(0, p_len, p_len/NUM_THREADS + 4), [&](auto range) {
    for (int i = range.begin(); i < range.end(); i++) {
      if (p_brand1[i] >= 260 && p_brand1[i] <= 267) {
        int key = p_partkey[i];
