...
void filter_probe_CPU(int* lo_off, int* dim_off1, int* dim_off2, int* dim_off3, int* dim_off4,
    ...
    for (int batch_start = start; batch_start < end_batch; batch_start += BATCH_SIZE) {
      ...
      for (int i = batch_start; i < batch_start + BATCH_SIZE; i++) {
        ...
        if (lo_off != NULL) lo_offset = lo_off[start_offset + i];
          ...
          // if (segment_group == NULL) lo_offset = start_offset + i;
          // else {
            int segment_idx = segment_group[i / SEGMENT_SIZE];
            lo_offset = segment_idx * SEGMENT_SIZE + (i % SEGMENT_SIZE);
          // }
        }
...
void filter_probe_group_by_CPU(int* lo_off, int* dim_off1, int* dim_off2, int* dim_off3, int* dim_off4,
  int* filter_col1, int* filter_col2, int compare1, int compare2, int compare3, int compare4,
  int* key_col1, int* key_col2, int* key_col3, int* key_col4, 
  int* aggr_col1, int* aggr_col2, int* group_col1, int* group_col2, int* group_col3, int* group_col4, int mode,
  int num_tuples, int* ht1, int dim_len1, int* ht2, int dim_len2, int* ht3, int dim_len3, int* ht4, int dim_len4,
...
// }
      ...
      // if (filter_col1 != NULL) {
      //   bool flag = (filter_col1[lo_offset] >= compare1 && filter_col1[lo_offset] <= compare2); //only for Q1.x
      //   if (!flag) continue;
      // }

      // if (filter_col2 != NULL) {
      //   bool flag = (filter_col2[lo_offset] >= compare3 && filter_col2[lo_offset] <= compare4); //only for Q1.x
      //   if (!flag) continue;
      // }
