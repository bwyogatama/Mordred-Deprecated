...
public:
  ...
  CPUGPUProcessing(size_t _cache_size, size_t _ondemand_size, size_t _processing_size, size_t _pinned_memsize) {
...
CPUGPUProcessing::call_pfilter_GPU(QueryParams* params, int** &off_col, int* &d_total, int* h_total, int sg, int select_so_far, cudaStream_t stream) {
  ...
  if (off_col == NULL) {
    ...
    off_col_out[0] = cm->customCudaMalloc(output_estimate);
...
CPUGPUProcessing::call_pfilter_CPU(QueryParams* params, int** &h_off_col, int* h_total, int sg, int select_so_far) {
  ...
  if (h_off_col == NULL) {
    ...
    off_col_out[0] = cm->customCudaHostAlloc(output_estimate);
